...
using MixingBuffer =
    std::array<std::array<float, FrameCombinerMy::kMaximumChannelSize>,
               FrameCombinerMy::kMaximumNumberOfChannels>;
...
void SetAudioFrameFields(rtc::ArrayView<const AudioFrame* const> mix_list,
  ...
  const size_t samples_per_channel = static_cast<size_t>(
      (sample_rate * webrtc::AudioMixerMy::kFrameDurationInMs) / 1000);
...
void MixFewFramesWithNoLimiter(rtc::ArrayView<const AudioFrame* const> mix_list,
                               AudioFrame* audio_frame_for_mixing) {
  std::cout << "DEBUG 5 " << mix_list.size() << std::endl;

  if (mix_list.empty()) {
    audio_frame_for_mixing->Mute();
    return;
  }
  std::cout << "DEBUG 66 " << mix_list.size() << std::endl;
...
void MixToFloatFrame(rtc::ArrayView<const AudioFrame* const> mix_list,
  ...
  RTC_DCHECK_LE(samples_per_channel, FrameCombinerMy::kMaximumChannelSize);
  RTC_DCHECK_LE(number_of_channels, FrameCombinerMy::kMaximumNumberOfChannels);
  ...
  for (size_t i = 0; i < mix_list.size(); ++i) {
    ...
    for (size_t j = 0; j < std::min(number_of_channels,
      ...
      for (size_t k = 0; k < std::min(samples_per_channel,
                                      FrameCombinerMy::kMaximumChannelSize);
           ++k) {
...
void RunLimiter(AudioFrameView<float> mixing_buffer_view, Limiter* limiter) {
  const size_t sample_rate = mixing_buffer_view.samples_per_channel() * 1000 /
                             AudioMixerMy::kFrameDurationInMs;
...
constexpr size_t FrameCombinerMy::kMaximumNumberOfChannels;
constexpr size_t FrameCombinerMy::kMaximumChannelSize;

FrameCombinerMy::FrameCombinerMy(bool use_limiter)
...
FrameCombinerMy::~FrameCombinerMy() = default;

void FrameCombinerMy::Combine(rtc::ArrayView<AudioFrame* const> mix_list,
                            size_t number_of_channels,
                            int sample_rate,
                            size_t number_of_streams,
                            AudioFrame* audio_frame_for_mixing) {
  ...
  std::cout << "DEBUG 2 " << samples_per_channel << std::endl;
  ...
  std::cout << "DEBUG 3 " << number_of_channels << std::endl;
  ...
  std::cout << "DEBUG 4 " << (number_of_streams <= 1) << std::endl;
