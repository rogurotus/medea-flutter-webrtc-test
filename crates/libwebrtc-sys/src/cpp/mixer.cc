...
struct AudioMixerMy::SourceStatus {
...
struct SourceFrame {
  ...
  SourceFrame(AudioMixerMy::SourceStatus* source_status,
              AudioFrame* audio_frame,
              bool muted)
  ...
  AudioMixerMy::SourceStatus* source_status = nullptr;
...
std::vector<std::unique_ptr<AudioMixerMy::SourceStatus>>::const_iterator
FindSourceInList(
    AudioMixerMy::Source const* audio_source,
    std::vector<std::unique_ptr<AudioMixerMy::SourceStatus>> const*
        audio_source_list) {
  return std::find_if(
      audio_source_list->begin(), audio_source_list->end(),
      [audio_source](const std::unique_ptr<AudioMixerMy::SourceStatus>& p) {
        return p->audio_source == audio_source;
      });
}
}  // namespace

struct AudioMixerMy::HelperContainers {
...
rtc::scoped_refptr<AudioMixerMy> AudioMixerMy::Create(
    ...
    bool use_limiter,
    int max_sources_to_mix) {
  return rtc::make_ref_counted<AudioMixerMy>(
      std::move(output_rate_calculator), use_limiter, max_sources_to_mix);
}

void AudioMixerMy::Mix(size_t number_of_channels,
                         AudioFrame* audio_frame_for_mixing) {
  TRACE_EVENT0("webrtc", "AudioMixerMy::Mix");
  RTC_DCHECK(number_of_channels >= 1);
  MutexLock lock(&mutex_);

  size_t number_of_streams = audio_source_list_.size();

  std::cout << " DEBUG1 " << number_of_streams << std::endl;
...
bool AudioMixerMy::AddSource(Source* audio_source) {
...
void AudioMixerMy::RemoveSource(Source* audio_source) {
  RTC_DCHECK(audio_source);
  MutexLock lock(&mutex_);
...
rtc::ArrayView<AudioFrame* const> AudioMixerMy::GetAudioFromSources(
...
bool AudioMixerMy::GetAudioSourceMixabilityStatusForTest(
    AudioMixerMy::Source* audio_source) const {
  MutexLock lock(&mutex_);
