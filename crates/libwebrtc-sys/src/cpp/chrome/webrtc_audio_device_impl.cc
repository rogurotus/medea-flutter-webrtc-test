...
// // #include "blink/public/platform/modules/webrtc/webrtc_logging.h"
// // #include "blink/renderer/modules/mediastream/processed_local_audio_source.h"
// // #include "blink/renderer/modules/webrtc/webrtc_audio_renderer.h"
...
void WebRtcAudioDeviceImpl::RenderData(
  ...
  //              sample_rate, "audio_delay_ms", audio_delay.InMilliseconds());
  {
    ...
    total_samples_duration_ += audio_bus->frames() * 1000000 / sample_rate; // micro
// #if DCHECK_IS_ON()
...
int32_t WebRtcAudioDeviceImpl::PlayoutDelay(uint16_t* delay_ms) const {
  // DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
  // base::AutoLock auto_lock(lock_);
  const int64_t output_delay_ms = output_delay_;
  // DCHECK_LE(output_delay_ms, std::numeric_limits<uint16_t>::max());
  *delay_ms = (uint16_t)(output_delay_ms);
  return 0;
}
...
void WebRtcAudioDeviceImpl::RemovePlayoutSink(
  ...
  // base::AutoLock auto_lock(lock_);
  std::lock_guard<std::mutex> auto_lock(lock_);
  for (auto it = playout_sinks_.begin(); it != playout_sinks_.end(); it++) {
    if (*it == sink) {
      playout_sinks_.erase(it);
      break;
    }
  }
}
