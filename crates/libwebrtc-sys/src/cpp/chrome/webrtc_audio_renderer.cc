...
#include "chrome/webrtc_audio_renderer.h"
...
// for details.
constexpr int kPowerMeasurementTimeConstant = 10;

// Time in seconds between two successive measurements of audio power levels.
constexpr int kPowerMonitorLogInterval = 15;
...
class SharedAudioRenderer : public WebMediaStreamAudioRenderer {
 public:
  ...
  using OnPlayStateRemoved =
      std::function<void(WebRtcAudioRenderer::PlayingState*)>;

  SharedAudioRenderer(
      WebMediaStreamAudioRenderer* delegate,
...
int WebRtcAudioRenderer::GetCurrentRenderTime() {
  // DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  // base::AutoLock auto_lock(lock_);
...
int WebRtcAudioRenderer::Render(int delay,
                                int delay_timestamp,
...
void WebRtcAudioRenderer::SendLogMessage(const std::string& message) {
