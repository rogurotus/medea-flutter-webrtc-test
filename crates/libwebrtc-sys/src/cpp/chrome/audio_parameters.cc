...
#include "chrome/audio_parameters.h"

#include <sstream>

#include "base/check_op.h"
#include "chrome/audio_bus.h"
#include "chrome/channel_layout.h"
#include "chrome/limits.h"
...
uint32_t ComputeAudioInputBufferSizeChecked(
    const AudioParameters& parameters,
    uint32_t shared_memory_count) {
  auto result = AudioBus::CalculateMemorySize(
...
uint32_t ComputeAudioOutputBufferSizeChecked(
    const AudioParameters& parameters) {
  uint32_t result = AudioBus::CalculateMemorySize(
      parameters.channels(), parameters.frames_per_buffer());
...
double AudioParameters::GetMicrosecondsPerFrame() const {
  return static_cast<double>(1000000) / sample_rate_;
}

int AudioParameters::GetBufferDuration() const {
  return static_cast<int64_t>(
      frames_per_buffer_ * 100000 /
