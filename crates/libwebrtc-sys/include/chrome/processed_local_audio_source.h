...
#include "media_stream_request.h"
...
#include "media_stream_audio_source.h"
...
class MODULES_EXPORT ProcessedLocalAudioSource final
 ...
 public:
  ...
  // configuration.
  webrtc::AudioProcessorInterface* GetAudioProcessor() const;
 ...
 private:
  ...
  LocalFrame* consumer_frame_;
  PeerConnectionDependencyFactory* dependency_factory_;

  // using SourceStoppedCallback =
  //     std::function<void(const WebMediaStreamSource& source)>;

  // using ConstraintsRepeatingCallback =
  //     std::function<void(WebPlatformMediaStreamSource* source,
  ...
  using KeepDeviceAliveForTransferCallback = std::function<void(bool)>;
  ...
  MediaStreamAudioProcessor* media_stream_audio_processor_;

  // Proxy for the audio processor when it's run in the Audio Service process,
  AudioServiceAudioProcessorProxy* audio_processor_proxy_;

  // The device created by the AudioDeviceFactory in EnsureSourceIsStarted().
  media::AudioCapturerSource* source_;
