...
#include "webrtc_audio_device_not_impl.h"
#include "webrtc_source.h"
#include "webrtc_audio_renderer.h"
...
class MODULES_EXPORT WebRtcAudioDeviceImpl
 ...
 public:
  ...
  std::string GetAuthorizedDeviceSessionIdForAudioRenderer();

  const blink::WebRtcAudioRenderer* renderer() const {
    return renderer_;
  }
  ...
  void RenderData(media::AudioBus* audio_bus,
                  ...
                  int audio_delay,
                  int* current_time
                  // const media::AudioGlitchInfo& glitch_info toodo
                  ) override;
 ...
 private:
  ...
  std::vector<ProcessedLocalAudioSource*> capturers_;

  // Provides access to the audio renderer in the browser process.
  blink::WebRtcAudioRenderer* renderer_ ;
  ...
  // // before it goes away.
  std::vector<blink::WebRtcPlayoutDataSource::Sink*> playout_sinks_;
  ...
  // // media::AudioGlitchInfo cumulative_glitch_info_ GUARDED_BY(lock_); toodo
  int total_samples_duration_;
  int total_playout_delay_;
  uint64_t total_samples_count_ = 0;
};
class temp : public rtc::RefCountedObject<WebRtcAudioDeviceImpl> {
